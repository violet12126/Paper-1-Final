...
def run_experiment(seed, num_epochs=60, visualize=False):
    ...
    model = CNN(
        ...
        base_channels=55,
        extra_conv_layers=0,
        use_res=False,  # 对应 use_attention: True
        dropout_rate=0.19322850860193252,
        kernel_size=5
    ).to(device)
    ...
    optimizer = optim.AdamW(
        model.parameters(),
        lr=0.000241,
        weight_decay=3.101304303035799e-06
    )
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_bar:
            ...
            train_bar.set_postfix({
                'loss': f'{loss.item():.4f}',
                'acc': f'{(predicted == labels).sum().item() / batch_size_current:.3f}',
        ...
        with torch.no_grad():
            for inputs, labels in val_bar:
                ...
                val_bar.set_postfix({
                    ...
                    'val_acc': f'{(predicted == labels).sum().item() / batch_size_current:.3f}'
                })
        val_loss /= val_total
...
if __name__ == "__main__":
    ...
    with open('experiment_results.txt', 'w', encoding='utf-8') as f:
        ...
        f.write("\n最优超参数配置:\n")
        f.write("base_channels: 44\n")
        f.write("lr: 0.00015257679533429273\n")
        f.write("dropout: 0.19322850860193252\n")
        f.write("weight_decay: 1.1481369329951204e-06\n")
        f.write("kernel_size: 3\n")
        f.write("extra_conv_layers: 0\n")
